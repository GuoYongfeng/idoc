...
> 就我自己而言，也是在学习和接触React的路上，但是我希望把我知道的这些和大家分享，避免大家走一些不必要的弯路。
...
- 这套技术实现，框架库代码压缩后大于200K,gzip后实际传输大小为60K+，更**适合大型的webapp**。
- 配合ES6的使用，可以大幅**提高编程效率**，降低很多之前的代码坑。
- 有很**优秀的开源组件库**可以使用：`Antd Materialui React-bootstrap Amaze-ui Sematic-ui`

当然啦，接触一些新事物也是有成本的。
- **快速学习部分新知识**，需要专业前端开发。
- **动态化输出展现**，即与服务端的同构，需要Nodejs起一个轻量的server结合运行。或者另一种方案是，服务端将模板和元数据结合后通过Restful API输出html片段的字符串，前端进行组装。
- **也会有未知的坑需要填**。
- 之前已经定了那种AMD加载的机制。
